...
class DGCRN(object):
    ...
    def __init__(self, model_name: str, learning_rate: float = 1e-3, batch_size: int = 5, n_hidden: int = 5,
                 num_nodes: int = None, num_external_features: int = 0, external_embedding_size: int = 5,
                 seq_len: int = 40, input_dim: int = 2, output_dim: int = 2):
        ...
        self.external_embedding_size = external_embedding_size
    ...
    def _make_graph(self):
        ...
        e = tf.placeholder('float', [self.batch_size, self.num_external_features], name='external-features')

        self.input = x
        self.label = y
        self.external_embeddings = e

        e = tf.contrib.layers.fully_connected(e, self.external_embedding_size)
        ...
        fussion = tf.concat([state, e], axis=1)
        pred = tf.contrib.layers.fully_connected(fussion, self.num_nodes * self.output_dim, activation_fn=None)
    ...
    def _define_loss(self):
        ...
        # # Learning rate decay with rate 0.1 every 50 epochs.
    ...
    def partial_fit(self, sess: Union[tf.Session, tf.InteractiveSession], training_epochs: int, display_step: int = 10,
        ...
        while epoch <= training_epochs:
            ...
            # 验证并保存验证损失最小的模型
            losses, total_start = list(), datetime.datetime.now()
            for idx, (test_data, test_label, e_data) in enumerate(validate_input_generator()):
                start_time = datetime.datetime.now()
    ...
    def predict(self, sess: Union[tf.Session, tf.InteractiveSession], input_generator: Callable[[], Generator],
        ...
        for idx, (test_data, test_label, e_data) in enumerate(input_generator()):
            prediction, mse, (pcc, _), mae = sess.run(
                [self.prediction, self.metrics['mse'], self.metrics['pcc'], self.metrics['mae']],
                feed_dict={self.input: test_data, self.label: test_label, self.external_embeddings: e_data})
            mses.append(mse)
            pccs.append(pcc)
            maes.append(mae)
